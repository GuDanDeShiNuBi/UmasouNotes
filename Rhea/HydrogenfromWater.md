...
- [x] 软件设计方面问题：1、软件未开启，设备运行，这阶段的部分数据是无法得到的。例如某一模块运行的时长无法获取
...
- [x] 首页当日用水量、发电量、输入功率存在问题，当设备开启，软件没看的情况下会出现数据丢失问题
- [x] 燃料电池、市电、光伏、水电解槽增加累计时间，对软件设计进行修改，不需要软件内对各模块累计时长的计算（当日运行时长=当日已累计时长+（当前时间-启动时间）
- [x] 实时数据增加燃料电池功率、光伏功率、市电功率
20221109
- [x] 增加退出提示窗口
- [x] 软件与设备断开连接提示问题。（首页没对plc数据操作，所以没有对plc连接进行监测，首页增加了一个plc数据读取）
- [x] 设备连接状态颜色设置，退出按钮增加背景框
- [x] 社会效益修改
  - [x] 每度电的CO2减排量为0.806kg,NO2的减排量为0.0028kg,SO2的减排量为0.0074kg,后面两个数字小，单位可以用g.
  - [x] 弃光电量=市电累计发电量
- [x] 设备编号不能重复，一个账号对应一个设备编号
- [x] 首页社会效益所需的累计市电用电量需要加上当日的市电用电量
  - [x] 登录时就更新historydata今日的累计数据
- [x] 实时监控页发电量改为发电功率，制氢量改为产氢效率
- [x] 折线图的单位添加
- [x] 使用说明书中图片更新
...
20221118
- [x] 折线图数据小数点位数需要控制为三位小数:float转double
- [x] 首页燃料电池发电量需测试，改为焦耳，w*60
20221123
- [x] 实时监控界面，发电效率、制氢效率的折线图时间刻度，当时间达到尾端时，更新时间轴刻度（按照当前时间）
